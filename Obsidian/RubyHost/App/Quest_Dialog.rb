...
class Quest_Dialog < DialogHost
  ...
  Objectifs = ["Parler à un PNJ", "Battre un PNJ", "Trouver objet", "Voir Pokémon", "Battre Pokémon", "Capturer Pokémon", "Obtenir oeuf", "Eclore oeuf"]
  Recompenses = ["Argent", "Objet"]
  ...
  def build_objectives
    ...
    if(@object.speak_to)
      @object.speak_to.each_with_index do |name, index|
        @objective_list << "Parler à #{name} > speak #{index}"
      end
    end
    if(@object.beat_npc)
      @object.beat_npc.each_with_index do |name, index|
        @objective_list << "Battre #{name} #{@object.beat_npc_amount[index]} fois > beat_npc #{index}"
      end
    end
    if(@object.items)
      @object.items.each_with_index do |item_id, index|
        @objective_list << "Trouver #{@object.item_amount[index]} #{GameData::Text._get(lang, 12, item_id)} > items #{index}"
      end
    end
    if(@object.see_pokemon)
      @object.see_pokemon.each_with_index do |pokemon_id, index|
        @objective_list << "Voir #{GameData::Text._get(lang, 0, pokemon_id)} > see #{index}"
      end
    end
    if(@object.beat_pokemon)
      @object.beat_pokemon.each_with_index do |pokemon_id, index|
        @objective_list << "Battre #{@object.beat_pokemon_amount[index]} #{GameData::Text._get(lang, 0, pokemon_id)} > beat #{index}"
      end
    end
    if(@object.catch_pokemon)
      @object.catch_pokemon.each_with_index do |pokemon_id, index|
        ...
        @objective_list << "Capturer #{@object.catch_pokemon_amount[index]} #{name} > catch #{index}"
      end
    end
    if(@object.get_egg_amount)
      @objective_list << "Obtenir #{@object.get_egg_amount} oeufs > egg 0"
    end
    if(@object.hatch_egg_amount)
      @objective_list << "Faire éclore #{@object.hatch_egg_amount} oeufs > hatch 0"
    end
  end
  ...
  def _add_quest
    if(check_text_before_adding(size = $game_data_quest.size, @lang_id, 45, 46))
      ...
      msgbox("Il n'y a pas assez de texte de nom ou de description pour les quêtes", self.class.to_s)
    end
  end
  
  def _del_quest
    if(confirm("Toute suppression peut entrainer des bugs dans le jeu.\nVoulez-vous vraiment supprimer cette quête ?", self.class.to_s))
