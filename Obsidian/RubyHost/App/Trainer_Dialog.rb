...
class Trainer_Dialog < DialogHost
  ...
  TEXT_SPLIT = "("
  TYPE_FORMAT = '%d - %s'
  Genders = ["Aléatoire", "Indéterminé (0)","Mâle (1)","Femelle (2)"]
  RandomForm = "Forme aléatoire"
  RandomAbility = "Talent aléatoire"
  DefaultAttack = "Défaut (0)"
  ...
  def _add_trainer
    if(check_text_before_adding(size = $game_data_trainer.size, @lang_id, 29, 47, 48))
      ...
      msgbox("Il n'y a pas assez de texte de classe, de phrase de victoire ou de défaite.", self.class.to_s)
    end
  end
  
  def _remove_trainer
    if(confirm("Toute suppression peut entrainer des bugs dans le jeu.\nVoulez-vous vraiment supprimer ce dresseur ?", self.class.to_s))
      if @group_id == $game_data_trainer.size - 1 and @group_id > 0
        ...
        msgbox("Le dresseur sera vidé.", self.class.to_s)
  ...
  def _remove
    if @object.team.size <= 1
      return msgbox("Les dresseurs doivent avoir au moins contenir un Pokémon...", self.class.to_s)
    end
