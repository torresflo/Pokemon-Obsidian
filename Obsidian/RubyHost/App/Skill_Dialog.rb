...
class Skill_Dialog < DialogHost
  ...
  Skill_TextTarget = [
      "Normale :adjacent_pokemon",                      #  eo eo ex / ux ao ax
      "Un adv. proche :adjacent_foe Ennemi adjacent",   #  eo eo ex / ux ax ax
      "Adv. proches :adjacent_all_foe",                 #  e! e! ex / ux ax ax
      "Côté adverse :all_foe",                          #  e! e! e! / ux ax ax
      "Plusieurs alliés/adv :adjacent_all_pokemon",     #  e! e! ex / ux a! ax
      "Tous côtés :all_pokemon",                        #  e! e! e! / u! a! a!
      "Soi :user",                                      #  ex ex ex / u! ax ax
      "Soi ou un allié :user_or_adjacent_ally",         #  ex ex ex / uo ao ax
      "Un allié :adjacent_ally",                        #  ex ex ex / ux ao ax
      "Côté allié :all_ally",                           #  ex ex ex / u! a! a!
      "Normale LP :any_other_pokemon",                  #  eo eo eo / ux ao ao
      "Un adv. au hasard :random_foe"                   #  e? e? e? / ux ax ax
    ]
  Critical = ["0 - Null", "1 - Normal 7%", "2 - Elevé 13%", "3 - 25%", "4 - Interdit 34%", "5 - Interdit 50%"]
  Category = ["1 - Physique", "2 - Spéciale", "3 - Statut"]
  Text_Method = ["Spéciale","Normale :s_basic","Statistique :s_stat","Statut :s_status","Jusqu'à 5 coups :s_multi_hit","Deux coups :s_2hits","OHKO :s_ohko","En deux tours :s_2turns","Changement de stat sur soi :s_self_stat","Status sur soi :s_self_statut"]
  Status = ["0 - Aucun", "1 - Poison", "2 - Paralisie", "3 - Brûlure", "4 - Sommeil", "5 - Gel", "? - Confusion", "? - Peur", "8 - Intoxication"]
  ...
  def _add
    if(check_text_before_adding(size = $game_data_skill.size, @lang_id, 6, 7))
      ...
      msgbox("Il n'y a pas assez de texte de nom ou de description.", self.class.to_s)
    end
  end
  
  def _del
    if(confirm("Toute suppression peut entrainer des bugs dans le jeu.\nVoulez-vous vraiment supprimer cette attaque ?", self.class.to_s))
