...
class Pokemon_Dialog < DialogHost
  ...
  TEXT_SPLIT = "("
  TYPE_FORMAT = '%d - %s'
  EXP_TYPES = ["0 - Rapide", "1 - Normal", "2 - Lent", "3 - Parabolique", "4 - Erratique", "5 - Fluctuante"]
  GROUPS = ["0 - Non défini", "1 - Monstrueux", "2 - Aquatique 1", "3 - Insectoïde", "4 - Aérien", "5 - Terrestre", "6 - Féerique", "7 - Végétal", "8 - Humanoïde", "9 - Aquatique 3", "10 - Minéral", "11 - Amorphe", "12 - Aquatique 2", "13 - Métamorph", "14 - Draconique", "15 - Inconnu"]
  Forms = ["0 - Commune"]
  Forms2 =  ["0 - Commune"]
  1.step(29) do |i| Forms<<"#{i} - Inexistante";Forms2<<"#{i} - Existante" end
  Forms<<"30 - Méga 1"
  Forms<<"31 - Méga 2"
  Forms2<<"30 - Méga 1/Def"
  Forms2<<"31 - Méga 2/Def"
  POKEMON_FORM_PATH = "Graphics\\pokedex\\PokeFront\\%03d_%02d.png"
  POKEMON_PATH = "Graphics\\pokedex\\PokeFront\\%03d.png"
  Gender=["Inconnu","Mâle","Femelle"]
  DayTime=["Nuit","Aube","Matin","Jour"]
  ...
  def _set_special_evl(value)
    ...
    msgbox("Erreur de syntaxe dans l'évolution spéciale.", self.class.to_s)
  end
  ...
  def _get_baby_list
    if @baby_list and (@baby_list.size) == $game_data_pokemon.size
      ...
      @baby_list = ["Aucun (0)"] + _pokemon_list
    end
    return @baby_list
  end
  ...
  def _check_evolve
    check_update_data
    ev = false
    str = "#{_get_pokemon_name} évolue en :"
    evolution_id = @object.evolution_id
    evolution_level = @object.evolution_level
    if(evolution_id * evolution_level > 0)
      begin
        str << "\r\n- #{GameData::Text._get(@lang_id,0,evolution_id)} au niveau #{evolution_level}"
        ev = true
      rescue TextDataError
        msgbox("Ce pokémon ne peut évoluer en Pokémon non défini : n°#{evolution_id} inexistant.", self.class.to_s)
      end
    end
    ...
    if(v and v.class == Array)
      v.each do |i|
        ...
        begin
          ...
          msgbox("Ce pokémon ne peut évoluer en Pokémon non défini... (Evolution spéciale : n°#{i[:trade] || i[:id]})", self.class.to_s)
        end
        #> Echange avec
        if(id = i[:trade_with])
          begin
  ...
  def _add
    if(check_text_before_adding(size = $game_data_pokemon.size, @lang_id, 0, 1, 2))
      ...
      msgbox("Il n'y a pas assez de texte de nom, d'espèce ou de description.", self.class.to_s)
    end
  end
  
  def _del
    if(confirm("Toute suppression peut entrainer des bugs dans le jeu.\nVoulez-vous vraiment supprimer ce Pokémon ?", self.class.to_s))
