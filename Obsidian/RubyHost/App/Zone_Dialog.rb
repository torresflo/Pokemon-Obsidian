...
class Zone_Dialog < DialogHost
  ...
  def _remove
    ...
    if(@zone_id != $game_data_zone.size - 1)
      return unless confirm("Attention, toute les zones suivante seront décallées de 1. Supprimer quand même ?", self.class.to_s)
    end
    if @object.groups.size > 0
      return unless confirm("Cette zone contient des groupes, voulez-vous la supprimer ?", self.class.to_s)
    end
