...
class Main_Dialog < DialogHost
  ...
  OpenProjectSTR = "Ouvrez un projet !"
  private
  #===
  ...
  def init_on_close(dialog)
    def dialog.on_close(hDlg)
      if(App.warn?)
        if MessageBox(
              hDlg,
              "Êtes-vous sûr de vouloir fermer l'éditeur ?",
              "Fermer l'éditeur",
  ...
  def _edit_ability
    ...
    if($game_data_abilities.size < (texts.size - 1) && texts.size <= GameData::Text.get_text_file(App.lang_id,5).size)
      ...
      msgbox("Les talents sont à jour.", "Information")
    end
  end
  ...
  def _open_project
    ...
    if(App.get_dir)
      if(!confirm("Êtes-vous sûr de vouloir changer de dossier d'édition ?","Changer de dossier"))
        return false
      end
    end
