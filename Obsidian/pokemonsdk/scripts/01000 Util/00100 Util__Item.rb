...
=end
    ...
    def util_item_open_party_sequence(extend_data, result_process)
      party = @team || $actors
      call_scene(GamePlay::Party_Menu, party, :item, extend_data, no_leave: false) do |scene|
        if $game_temp.in_battle && scene.return_data != -1
          GamePlay::Bag.from(self).battle_item_wrapper = extend_data
          @running = false
          next
        elsif scene.return_data != -1
